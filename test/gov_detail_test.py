...
def get_detail_info(url):
    ...
    if response.status_code == 200:
            ...
            try:
                # 提取项目编号到项目名称之间的内容
                project_number = tree.xpath('//text()[contains(., "项目编号")]')
                if project_number:
                    project_number_text = project_number[0].split("项目名称")[0].strip()
                else:
                    project_number_text = ''
                # 或者更精确地提取项目编号
                project_title = tree.xpath('//text()[contains(., "项目名称")]')
                if project_title:
                    project_title_text = project_title[0].split("预算金额")[0].strip()
                else:
                    project_title_text = ''
                print(f"提取的项目名称: {project_title_text}")
                print(f"提取的项目编号: {project_number_text}")
            ...
            return project_number_text, project_title_text, total_content
    else:
...
if __name__ == "__main__":
    url = 'https://zfcg.czj.ningbo.gov.cn/project/zcyNotice_view.aspx?Id=202505061900071919708752873918464'  # 替换为实际的 URL
